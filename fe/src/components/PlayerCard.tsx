...
import { IconX, IconO, IconClock } from "./Icons";
...
interface PlayerCardProps {
  ...
  dangerThreshold?: number; // default 10
}
...
export const PlayerCard: React.FC<PlayerCardProps> = ({
  ...
  const isX = player === "X";
  const isDanger = isActive && !winner && timeRemaining <= dangerThreshold;

  // Tailwind-safe classes (no dynamic template strings)
  const activeClasses = isX
    ? "bg-slate-800/90 border-cyan-500/50 ring-1 ring-cyan-500/30"
    : "bg-slate-800/90 border-rose-500/50 ring-1 ring-rose-500/30";

  const idleClasses = "bg-slate-900/50 border-slate-800 opacity-90";

  const winnerClasses =
    "bg-gradient-to-br from-amber-900/20 to-slate-900 border-amber-500/50 shadow-[0_0_30px_rgba(245,158,11,0.2)]";

  const avatarClasses = isX
    ? "bg-cyan-950/50 border-cyan-500/30"
    : "bg-rose-950/50 border-rose-500/30";

  const timerActiveClasses = isX
  ...
  return (
    <div
      className={[
        ...
        isActive && !winner ? activeClasses : idleClasses,
        isWinner ? winnerClasses : "",
      ].join(" ")}
      style={
        {
          // vẫn giữ shadow-color nếu bạn cần (không phải tailwind)
          "--shadow-color": isX ? "6,182,212" : "244,63,94",
    ...
    >
      ...
      <div
        className={[
          "absolute -right-4 -top-4 w-24 h-24 blur-3xl rounded-full pointer-events-none",
          isX ? "bg-cyan-500/10" : "bg-rose-500/10",
        ].join(" ")}
      />
      ...
      <div className="relative">
        ...
        {isActive && !winner && (
          <span className="absolute -bottom-1 -right-1 flex h-4 w-4">
            ...
            <span className="relative inline-flex rounded-full h-4 w-4 bg-green-500 border-2 border-slate-900" />
          </span>
        )}
      </div>
      ...
      <div className="flex-grow text-left xl:text-center xl:w-full">
        <div className="flex flex-col xl:items-center">
          <h3 className="text-slate-100 font-bold text-lg tracking-tight truncate max-w-[170px]">
            {name}
          </h3>
